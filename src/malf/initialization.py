...
范围声明（第一刀，只做 golden fixture 驱动的部分）：
- 只实现 **up 方向**的干净序列 `H0 → L1 → H2, H2 > H0`。
- **down 方向**（`L0 → H1 → L2, L2 < L0`）：结构上与 up 对称，但目前没有专门的
  down 方向 golden fixture 驱动这条路径，为避免"看起来对称就当它对"的偷懒，
  显式 `NotImplementedError`——等第二条 fixture（反向）落地再补。
- **【填洞 C-07】H0 替换**（H0 确认后、L1 出现前，又来一个更高的 H）：
  spec 只说"更高的 H 可替换 H0，且替换后需重新评估条件"，但没规定替换后
  L1 的候选范围如何变化（是否只认替换点之后的 L，还是可以是更早的 L）。
  这是一处真正的规格模糊，本刀不猜——**任何在 L1 确认前出现的第二个 H**
  （无论高低）都显式报错，逼这条路径必须有专门 fixture 才能实现。
- **L1 替换**（L1 确认后、H2 确认前，又来一个更低的 L）：spec 完全没提这一支
  是否允许替换 guard 候选。同样显式报错，不猜。

这三处留白已记入 docs/BUILD-PLAN.md「已发现待处理」，不在代码里悄悄拍死。
"""
...
def find_initial_wave(pivots_in_confirm_order: Sequence[Pivot]) -> InitialWaveResult:
    """在按 confirm_bar_dt 排序的 pivot 序列上判定 initial wave 是否成立（D18/O6，up 方向）。
    ...
    不是错误。只有真正未实现的分支（down 方向 / H0 替换 / L1 替换）才抛
    ...
    if first.pivot_type == PivotType.L:
        raise NotImplementedError(
            "initial down 方向（L0→H1→L2<L0）暂未实现：无专门 fixture 驱动，"
            "不猜对称性直接实现。"
        )
